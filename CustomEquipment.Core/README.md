...
У одного оружия может быть несколько записей с одинаковым `trigger`:
при каждом срабатывании выбирается ровно один включённый вариант с равной
вероятностью и его собственной громкостью. Последовательные повторы возможны.
Это также работает для массива `WeaponItemBase.Sounds` в C#-определении оружия.
Миграция `20260905120000_AllowWeaponSoundVariants` сохраняет все записи,
заменяя старые уникальные индексы на `(weapon_id, trigger, event_name)`.
События можно повторно использовать у другого оружия или триггера.
Откат миграции будет отклонён, если новые варианты нарушают старые ограничения;
он не удаляет варианты автоматически.
