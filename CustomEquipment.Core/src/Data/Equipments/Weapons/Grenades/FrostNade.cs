...
internal sealed class FrostNade(GameplayItemCatalog catalog)
    : ManagedGrenadeItemBase(catalog, GameplayItemKeys.FrostNade)
{
    ...
    public override void OnDetonate(IPlayer thrower, Vector position)
    {
        ...
        var freezeSettings = new FreezeSettings(settings.Duration, settings.DamageReduction);

        foreach (var player in players)
        {
            effectService.ApplyEffect<Freeze>(thrower, player, freezeSettings);
        }
    }
}
