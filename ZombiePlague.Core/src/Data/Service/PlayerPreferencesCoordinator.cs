...
)
{
    ...
    public void Initialize(IPlayer player)
    {
        ...
        _loadedPlayers.Remove(player.SteamID);

        RunDatabaseTask(LoadAsync(player.SteamID, defaults));
    }

    public void SaveAndRemove(IPlayer player)
    {
        var steamId = player.SteamID;
        var wasLoaded = _loadedPlayers.Remove(steamId);

        if (CanSave(player) &&
            wasLoaded &&
            playerStore.TryGet(player, out var preferences))
        {
    ...
    private async Task LoadAsync(ulong steamId, PlayerPreferences defaults)
    {
        ...
        catch (Exception exception)
        {
            core.Logger.LogError(
                exception,
                "Failed to load preferences for SteamID {SteamId}. Defaults will be used.",
                steamId
            );
        }
    }
    ...
    )
    {
        ...
        // Не меняем класс живому игроку посреди игры. До первого спавна роль можно обновить безопасно.
