...
internal static class AbilityParameters
{
    public static IAbilityConfig Parse(ZombieAbilityDefinition definition)
    {
        ...
        {
            "heal" => typeof(HealConfig),
            "leap" => typeof(LeapConfig),
            "blind" => typeof(BlindConfig),
            "charge" => typeof(ChargeConfig),
            "trap" => typeof(TrapConfig),
    ...
    public static (double Minimum, double Maximum) Bounds(
        string name
    ) => name switch
    {
        ...
        "UpdateIntervalSeconds" => (0.01, 0.1),
        "ProjectileSpeed" => (1, 5000),
        "ProjectileGravity" => (0, 5000),
        "MaxDistance" => (1, 10000),
        "SpawnOffset" => (0, 500),
        "ExplosionRadius" => (1, 2000),
        "KnockbackPower" => (0, 5000),
        "KnockbackUp" => (0, 5000),
        "ExplosionParticleDuration" => (0.1, 30),
        "ExplosionSurfaceOffset" => (0, 100),
        "ChargeTime" => (1, 120),
        "CooldownTime" => (0, 3600),
        ...
        "BeamWidth" => (0.01, 20),
        "BurnMaxDamage" => (0, 10000),
        "BurnDuration" => (0.5, 60),

        "LiveDuration" or "EffectDuration" =>
            (0.1, 300),

        _ when name.Contains("Color") ||
               name.StartsWith("Alpha") =>
            (0, 255),

        _ when name.StartsWith("DurationEffect") ||
               name.StartsWith("HoldTimeEffect") =>
            (0, 60_000),
