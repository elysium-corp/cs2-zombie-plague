...
internal sealed record ShopHudAppearance
{
    ...
    public string PageAnimation { get; init; } = "scroll_x";
    public string AnimationSpeed { get; init; } = "normal";
    public string Highlight { get; init; } = "rarity";
    public string HoverHighlight { get; init; } = "rarity";
    ...
    internal static ShopHudAppearance Parse(string? json)
    {
        ...
        if (!new[] { "elysium", "minimal", "tactical" }.Contains(value.Theme) ||
            ...
            !new[] { "none", "fade", "slide", "zoom", "rise", "unfold", "drift" }.Contains(value.OpenAnimation) ||
            !new[] { "none", "fade", "slide", "zoom", "rise", "unfold", "drift" }.Contains(value.CloseAnimation) ||
            !new[] { "none", "fade", "slide", "scroll_x", "scroll_y", "zoom" }.Contains(value.PageAnimation) ||
            !new[] { "fast", "normal", "slow" }.Contains(value.AnimationSpeed) ||
            !new[] { "none", "rarity", "accent" }.Contains(value.Highlight) ||
            !new[] { "none", "rarity", "accent", "mint", "gold", "violet", "blue", "red" }.Contains(value.HoverHighlight) ||
    ...
    internal IEnumerable<(string Group, string Class)> Classes()
    {
        ...
        yield return ("hoverHighlight", "Hover_" + HoverHighlight);
