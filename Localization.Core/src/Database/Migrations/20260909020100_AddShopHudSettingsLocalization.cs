...
[Migration("20260909020100_AddShopHudSettingsLocalization")]
internal sealed class AddShopHudSettingsLocalization : Migration
{
    protected override void Up(MigrationBuilder migrationBuilder) => migrationBuilder.Sql("""
        CREATE TEMP TABLE shop_hud_settings_seed (key VARCHAR(191), ru TEXT, en TEXT) ON COMMIT DROP;
        INSERT INTO shop_hud_settings_seed VALUES
            ('Shop.Hud.Settings.Size', 'Размер меню', 'Menu size'),
            ('Shop.Hud.Settings.Loading', 'Загрузка настройки…', 'Loading preference…'),
            ('Shop.Hud.Settings.Saving', 'Сохранение…', 'Saving…'),
            ('Shop.Hud.Settings.Failed', 'Ошибка сохранения · нажмите размер ещё раз', 'Save failed · select a size to retry');
        INSERT INTO localization.entries (key, description, is_critical, parameters)
        SELECT seed.key, 'Настройки HUD магазина', FALSE, '[]'::jsonb FROM shop_hud_settings_seed seed
        ...
        FROM shop_hud_settings_seed seed
